...
def test_exports_heterogeneous_native_images_in_one_zarr_hierarchy(tmp_path: Path) -> None:
    ...
    velocity_analysis.result.table = pd.DataFrame(
        {"time": [0.0, 1.0], "velocity": [2.5, 3.5]}
    )
    ...
    for index, source in enumerate(images):
        child = destination / "images" / f"image_{index:03d}"
    ...
    first = zarr.open_group(str(destination / "images" / "image_000"), mode="r")
    assert len(first.attrs["ome"]["multiscales"][0]["datasets"]) >= 2
    assert first["0"].chunks == (64, 64)
    assert first["1"].chunks == (32, 32)
    third = zarr.open_group(str(destination / "images" / "image_002"), mode="r")
    assert len(third.attrs["ome"]["multiscales"][0]["datasets"]) == 1

    velocity = pd.read_csv(destination / "acqstore" / "tables" / "velocity.csv")
    sum_intensity = pd.read_csv(destination / "acqstore" / "tables" / "sum_intensity.csv")
...
def test_exports_reference_as_independent_ome_zarr_with_sidecar_geometry(
    ...
    assert manifest["images"][0]["reference_image"] == "images/image_000/reference"
    reference_path = destination / "images" / "image_000" / "reference"

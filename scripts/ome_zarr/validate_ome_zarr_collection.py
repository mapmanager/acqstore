...
Run from the AcqStore repository with its UV environment::
    ...
    uv run python scripts/ome_zarr/validate_ome_zarr_collection.py
"""
...
# OME_COLLECTION = Path(
#     "/Users/cudmore/Desktop/tmp/ome-zarr-collections/diameter_sample_data.ome.zarr"
# )

OME_COLLECTION = Path(
    "/Users/cudmore/Desktop/tmp/ome-zarr-collections/velocity-sample-data.ome.zarr"
)

EXPECTED_FORMAT = "acqstore-multi-image-ome-zarr"


def main() -> None:
    """Validate the collection contract, child metadata, and pyramid data."""
    root = OME_COLLECTION.expanduser().resolve(strict=True)
    ...
    root_status = "RECOGNIZED" if root_validation.returncode == 0 else "NOT RECOGNIZED"

    for entry in image_entries:
        image_id = str(entry["id"])
        child = _contained_path(root, entry["path"])
    ...
    print("Manifest: PASS")
    print(f"Root OME model: {root_status}")
...
def _load_manifest(root: Path) -> dict[str, object]:
    ...
    """
    manifest_path = root / "acqstore" / "manifest.json"
    ...
    for entry in images:
        ...
        _contained_path(root, entry.get("sidecar")).resolve(strict=True)
        _contained_path(root, entry.get("native_manifest")).resolve(strict=True)
        reference_path = entry.get("reference_image")
...
def _run_external_validator(executable: Path, target: Path) -> subprocess.CompletedProcess[str]:
    ...
    if not executable.is_file():
        raise FileNotFoundError(
            f"Validator is not installed in AcqStore's environment: {executable}"
        )
